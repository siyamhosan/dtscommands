...
parentPort?.on('message', (path: string, of: string) => {
  Compiler(path, of).then(() => {
    parentPort?.postMessage('done')
  })
})

export async function Compiler (path: string, of: string): Promise<void> {
...
`
  ...
  await exec(`eslint --fix ./.bundle/${of}-bundled.ts`)
